...
var uploadvideo141 = function(app){
  app.post('/uploadvideo141', upload.array('file',2), function(req, res, next) {
    ...
    for(var i=0;i<2;i++){
      if(req.files[i].originalname=='video'){
        var videoPath={
    ...
    if(database){
      ...
      VIDEO.save(async (err,result)=>{
        ...
        output.status=100;
        res.send(output);
        fs.readFile(thumbnailPath.path, 'base64', function (err, data) {
          if (!err) {
            var params_tn = {
                Bucket      : bucket,
                Key         : thumbnailPath.name,
                Body        : fs.createReadStream(thumbnailPath.path),
                ContentType : thumbnailPath.mimeType,
                ACL: 'public-read'
            };
            s3.putObject(params_tn, function(err, data) {
              var output={};
              fs.unlink(thumbnailPath.path,()=>{console.log('thumbnail unlink')})
    
              if (!err) {
                var aws_location_tn=`https://${bucket}.s3.ap-northeast-2.amazonaws.com/${thumbnailPath.name}`;
                var post_id = result._doc._id;
                var compressedPath = 'uploads/files/cp_'+videoPath.name;
            
                ffmpeg(videoPath.path)
                  ...
                  fs.readFile(compressedPath, 'base64', function (err, data) {
                    if (!err) {
                      var params = {
                          ...
                          ContentType : videoPath.mimeType,
                          ACL: 'public-read'
                      };
                      s3.putObject(params, function(err, data) {
                        var output={};
                        fs.unlink(`uploads/files/${videoPath.name}`,()=>{console.log('fsunlink')})
                        fs.unlink(`uploads/files/cp_${videoPath.name}`,()=>{console.log('fsunlink_new')})
                        if (!err) {
                          var aws_location=`https://${bucket}.s3.ap-northeast-2.amazonaws.com/${videoPath.name}`;
                          database.VideoModel.update({_id:post_id},{
                            show,
                            roll:aws_location,
                            thumb:aws_location_tn
                          },async (err)=>{
                            if(err){
                            ...
                            var my_id=my_profile[0]._doc._id.toString();
                            var group_id_arr=[];
                            if(groupList.length!==0){
                              var find_group_id= await database.FollowModel.find({group:{$in:groupList},_id:{$ne:my_id}}).limit(500);
                            ...
                            push_data.map(em=>{
                              fcm.send(em, function(err, response) {
                                ...
                                console.log('uploadvideo141: push success')
                              }); 
                            })
                          });
                        }else {
